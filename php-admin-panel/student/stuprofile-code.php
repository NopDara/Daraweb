...
if (isset($_POST['updateUser'])) {
    ...
    if ($user['status'] != 200) {
        redirect('stu-edit.php?id=' . $userId, 'No such id found');
    }
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] == UPLOAD_ERR_OK) {
        ...
        if (!move_uploaded_file($_FILES['photo']['tmp_name'], $photoPath)) {
            ...
            header("Location: stu-edit.php?error=$em&user_id=$user_id&photoPath=$photoPath");
            exit;
        }
    } else {
    ...
    if (!empty($password)) {
        ...
        // No new password provided, so don't update the password field
        $passwordUpdate = "";
    }
    ...
    if ($results) {
        ...
        redirect('stuprofile.php', 'Updated Successfully');
    } else {
        redirect('stu-edit.php?id=' . $userId, 'Something went wrong while updating user');
    }
}
?>